...
| P2  | `screenshot` and `video` settings         | Adopted                 | v0.2    | —                                                                |
| P3  | `isVisible()` anti-pattern warning        | Adopted                 | v0.2    | —                                                                |
...
| P6  | No ESLint in generated framework          | Adopted                 | v0.2    | —                                                                |
| P7  | Chromium-only configuration               | Intentionally Different | —       | [ADR-010](adr/ADR-010-single-browser-default.md)                 |
| P8  | No `testIdAttribute` configured           | Adopted                 | v0.2    | —                                                                |
| P9  | No soft assertions demonstrated           | Adopted                 | v0.2    | —                                                                |
...
The following Playwright recommendations are adopted in the generated framework.
...
| `screenshot` / `video`        | `screenshot: "only-on-failure"`, `video: "retain-on-failure"` — artifacts on failure only |
| `isVisible()` anti-pattern    | Explicit JSDoc in `BasePage.isVisible()` prohibits use as an assertion                    |
| ESLint + no-floating-promises | `eslint.config.mjs` generated with `@typescript-eslint/no-floating-promises: "error"`     |
| `testIdAttribute`             | `testIdAttribute: "data-testid"` surfaced in `playwright.config.ts` with a change guide   |
| `expect.soft()`               | Demonstrated in generated `README.md` with a multi-element verification example           |
| Cross-browser opt-in          | Firefox and WebKit projects included as commented-out blocks in `playwright.config.ts`    |
...
| ------- | ----------------------------------------------------------------------------------------- |
| v0.1    | Trace Viewer, POM, async/await, Locator API, TypeScript, ENV wrapper, Fixture-based setup |
